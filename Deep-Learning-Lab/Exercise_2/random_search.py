...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        ...
        #print("the x train is", self.x_train.shape)
        #print("the y train is", self.y_train.shape)
    ...
    def get_configspace():
        ...
        batch_size = CSH.UniformFloatHyperparameter('batch_size', lower = 16, upper = 128, default_value = 50, log = True)
        num_filters = CSH.UniformIntegerHyperparameter('num_filters', lower=8, upper=64, default_value=16, log=True)
        size_filter = CSH.UniformIntegerHyperparameter('filter_size', lower=3, upper=5, default_value=5, log=False)

        config_space.add_hyperparameters([lr,batch_size,num_filters, size_filter])
...
validation_accuracy, model, train_accuracy = train_and_validate(x_train, y_train, x_valid, y_valid, epochs, lr, num_filters, batch_size, filter_size)
